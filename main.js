(function () {
  ...
  const EMAILJS_PUBLIC_KEY = 'wvo7Smary0lSa1kDO';
  ...
  if (demoBackdrop && isIndexPage) {
    ...
    if (demoClose) demoClose.addEventListener('click', closeDemoPopup);
    ...
    /* Dynamic enrolled count: 12 to 49, increments by 1 on each reload */
    let enrolledCount = parseInt(localStorage.getItem('mma_enrolled_count'), 10);
    if (isNaN(enrolledCount) || enrolledCount < 12) {
      enrolledCount = 12;
    } else if (enrolledCount < 49) {
      enrolledCount += 1;
    }
    localStorage.setItem('mma_enrolled_count', enrolledCount);
    const urgencyTextEl = document.getElementById('dpUrgencyText');
    if (urgencyTextEl) {
      urgencyTextEl.textContent = `${enrolledCount} parents are already enrolled`;
    }
    ...
    if (demoForm) {
      demoForm.addEventListener('submit', e => {
        ...
        const centreInput = document.getElementById('dp-centre');
        const centre = centreInput ? centreInput.value.trim() : '';
        ...
        if (!time) {
          const timeBtn = document.getElementById('dp-time-btn');
          if (timeBtn) {
            timeBtn.scrollIntoView({ behavior: 'smooth', block: 'center' });
            timeBtn.style.borderColor = 'var(--red)';
            setTimeout(() => { timeBtn.style.borderColor = ''; }, 1500);
          }
          return;
        }
